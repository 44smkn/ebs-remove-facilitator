...
var tasks = map[string]func(string) error{
	"bin/ebspv-eraser": func(exe string) error {
		ldflags := os.Getenv("GO_LDFLAGS")
		ldflags = fmt.Sprintf("-X github.com/44smkn/ebspv-eraser/pkg/build.Version=%s %s", version(), ldflags)
		ldflags = fmt.Sprintf("-X github.com/44smkn/ebspv-eraser/pkg/build.Date=%s %s", date(), ldflags)
		return run("go", "build", "-trimpath", "-ldflags", ldflags, "-o", exe, "./cmd/ebspv-eraser")
	},
...
func version() string {
	if versionEnv := os.Getenv("EBSPV_ERASER_VERSION"); versionEnv != "" {
		return versionEnv
	}
